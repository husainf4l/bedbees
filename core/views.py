...
from .data.demo_accommodations import (
    demo_accommodations_data,
    get_featured_accommodations,
    get_guest_favorite_accommodations,
    get_unique_accommodations,
    get_popular_accommodations,
)


@login_required
def manage_listings(request):
    """View to manage and publish user's listings"""
    accommodations = Accommodation.objects.filter(host=request.user).order_by(
        "-created_at"
    )
    tours = Tour.objects.filter(host=request.user).order_by("-created_at")

    # Get total counts for all listings (for debugging)
    ...
    return render(request, "core/manage_listings.html", context)
...
def home(request):
    ...
    accommodations = (
        Accommodation.objects.filter(is_published=True, is_active=True)
        .select_related("host")
        .prefetch_related("photos")[:6]
    )
    tours = (
        Tour.objects.filter(is_published=True, is_active=True)
        .select_related("host")
        .prefetch_related("photos")[:6]
    )
    ...
    if search_type == "tours":
        ...
        if min_price and max_price:
            results = results.filter(
                base_price__gte=min_price, base_price__lte=max_price
            )
...
def tours(request, category=None):
    ...
    real_tours = Tour.objects.filter(is_published=True, is_active=True).select_related(
        "host"
    )
    ...
    if search_query:
        ...
        for tour in demo_tours:
            ...
            if (
                query_lower in tour["title"].lower()
                or query_lower in tour["location"].lower()
                or query_lower in tour["description"].lower()
                or query_lower in tour["category"].lower()
                or any(query_lower in tag.lower() for tag in tour["tags"])
            ):
    ...
    display_category = (
        category_names.get(category, "Explore All Experiences")
        if category
        else "Explore All Experiences"
    )
            ...
            if user is not None:
                ...
                try:
                    ...
                    if profile.is_host:
                        messages.success(
                            request,
                            f"Welcome back, {username}! Redirecting to your host dashboard.",
                        )
                        return redirect("core:hostdashboard")
                    else:
                        messages.success(
                            request,
                            f"Welcome back, {username}! Explore amazing experiences.",
                        )
                        return redirect("core:dashboard")  # Traveler dashboard
                except UserProfile.DoesNotExist:
...
def signup(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Traveler account created successfully! Welcome, {user.username}! Start exploring amazing experiences.",
            )
            return redirect("core:home")  # Redirect travelers to home page
        else:
...
def accommodations(request):
    ...
    # Demo accommodations data from our new data file
    demo_accommodations_list = list(demo_accommodations_data.values())

    # Handle search functionality
    search_query = request.GET.get("q", "").strip()
    accommodations = demo_accommodations_list
    ...
    if search_query:
        ...
        for accommodation in demo_accommodations_list:
            # Search in name, location, description, type, and amenities
            if (
                query_lower in accommodation["name"].lower()
                or query_lower in accommodation["location"].lower()
                or query_lower in accommodation["description"].lower()
                or query_lower in accommodation["type"].lower()
                or any(
                    query_lower in amenity.lower()
    ...
    if accommodation_type and not search_query:
        accommodations = [
            acc for acc in accommodations if acc["type"] == accommodation_type
        ]
    ...
    combined_accommodations = (
        list(real_accommodations) + accommodations
        if real_accommodations.exists()
        else accommodations
    )
...
def accommodation_detail(request, id):
    ...
    if str(id) in demo_accommodations_data:
        # Get the accommodation from our new demo data
        accommodation = demo_accommodations_data.get(str(id))
        ...
        for idx, photo_url in enumerate(photos_urls):
            photo_dict = {
                "id": idx,
                "image_url": photo_url,
                "alt_text": f"{accommodation.get('name', '')} photo {idx+1}",
                "title": f"{accommodation.get('name', '')} - Image {idx+1}",
                "caption": "",
                "display_order": idx,
                "is_hero": idx < 6,
                "media_type": "image",
    ...
    photos = AccommodationPhoto.objects.filter(
        accommodation=accommodation, visibility__in=["public", "hidden"]
        ...
        hero_photos_json.append(
            {
                ...
                or photo.title
                or f"{accommodation.property_name} photo",
                "title": photo.title,
...
def book_accommodation(request, id):
    ...
    demo_accommodations = [
        ...
        },
        {
            ...
            "check_out_time": "10:00",
            "property_highlights": [
                "Historic 6th arrondissement",
                "Walking distance to Louvre",
    ...
    if not accommodation:
        try:
            ...
            accommodation = {
                ...
                "room_type": "Standard Room",  # Default as it's not stored
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f'Booking confirmed for {accommodation["name"]}! A confirmation email has been sent to {email}.',
        )
        return redirect("core:dashboard")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            profile, created = UserProfile.objects.get_or_create(
                user=user, defaults={"is_host": True}
            )
            ...
            messages.success(
                request,
                f"Host account created successfully! Welcome to BedBees, {user.first_name or user.username}!",
            )
            return redirect(
                "core:hostdashboard"
            )  # Always redirect hosts to host dashboard
        else:
            messages.error(request, "Please correct the errors below.")
    else:
...
def hostdashboard(request):
    ...
    profile, created = UserProfile.objects.get_or_create(
        user=request.user, defaults={"is_host": True}
    )
    ...
    if not profile.is_host:
        messages.warning(
            request,
            "This page is only accessible to hosts. Redirecting to traveler dashboard.",
        )
        return redirect("core:dashboard")
...
def host_profile(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Your profile has been updated successfully!")
            return redirect("core:host_profile")
        else:
            messages.error(request, "Please correct the errors below.")
    else:
        form = HostProfileForm(instance=profile, user=request.user)
...
def upload_profile_photo(request):
    """AJAX view to handle profile photo uploads"""
    try:
        profile, created = UserProfile.objects.get_or_create(
            user=request.user, defaults={"is_host": False}
        )

        # Only update the profile_picture field
...
def logout_view(request):
    ...
    return redirect("core:home")  # Always redirect to home after logout
...
def create_accommodation(request):
    ...
    if request.method == "POST":
        ...
        property_features_str = (
            ",".join(property_features_list) if property_features_list else ""
        )

        nearby_landmarks_list = request.POST.getlist("nearby_landmarks")
        nearby_landmarks_str = (
            ",".join(nearby_landmarks_list) if nearby_landmarks_list else ""
        )
        ...
        if form.is_valid():
            ...
            if publish_action == "publish":
                ...
                if accommodation.country:
                    from .views_publishing import normalize_location
                    accommodation.country = normalize_location(accommodation.country)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if publish_action == "publish":
                ...
                if tour.country:
                    from .views_publishing import normalize_location
                    tour.country = normalize_location(tour.country)
            ...
            for index, photo in enumerate(photos):
                TourPhoto.objects.create(
                    tour=tour, image=photo, is_primary=(index == 0), order=index
                )
            ...
            if publish_action == "publish":
                ...
                messages.success(
                    request,
                    f'🎉 SUCCESS! "{tour.tour_name}" is now LIVE and published! '
                ...
                messages.success(
                    request,
                    f'✅ "{tour.tour_name}" saved as draft! '
                    f'Go to "My Listings" and click "Publish Now" when you\'re ready to go live.',
                )
            return redirect("core:hostdashboard")
        else:
...
def bookings(request):
    ...
    if filter_status != "all":
        filtered_bookings = [
            b for b in filtered_bookings if b["status"] == filter_status
        ]
    if filter_type != "all":
    ...
    upcoming_bookings = len(
        [b for b in filtered_bookings if b["status"] in ["confirmed", "pending"]]
    )
    total_spent = sum(
        b["total_price"] for b in filtered_bookings if b["status"] == "confirmed"
    )
    ...
    if search_query:
        ...
        for country in demo_countries:
            ...
            if (
                ...
                or query_lower in country["description"].lower()
                or any(
                    query_lower in attraction.lower()
                    for attraction in country["attractions"]
                )
            ):
...
def destinations(request):
    ...
    if search_query:
        ...
        for destination in demo_destinations:
            ...
            if (
                query_lower in destination["name"].lower()
                or query_lower in destination["description"].lower()
                or any(
                    query_lower in attraction.lower()
...
def wishlist(request):
    ...
    accommodations = [
        item for item in demo_wishlist_items if item["type"] == "accommodation"
    ]
...
def country_detail(request, country):
    ...
    if not country_data:
        ...
        return redirect("core:countries")

    # Query real accommodations from database for this country
    real_accommodations = (
        Accommodation.objects.filter(
            country__iexact=country_data["name"], is_published=True, is_active=True
        )
        .select_related("host")
        .prefetch_related("photos")
    )
        ...
        real_accommodation_list.append(
            {
                ...
                "image": (
                    primary_photo.get_image_url()
                    if primary_photo
                    else "https://images.unsplash.com/photo-1539020140153-e365f8dc0c7a?ixlib=rb-4.0.3&auto=format&fit=crop&w=2070&q=80"
                ),
    ...
    if real_accommodation_list:
        country_data["accommodations"] = real_accommodation_list + country_data.get(
            "accommodations", []
        )
...
def bulk_activate_listings(request):
    ...
    try:
        ...
        if not listing_ids:
            return JsonResponse(
                {"success": False, "error": "No listings selected"}, status=400
            )

        if listing_type == "accommodation":
            updated = Accommodation.objects.filter(
                id__in=listing_ids, host=request.user
            ).update(is_active=True)
        elif listing_type == "tour":
            updated = Tour.objects.filter(id__in=listing_ids, host=request.user).update(
                is_active=True
            )
        else:
            return JsonResponse(
...
def bulk_delete_listings(request):
    ...
    try:
        ...
        if listing_type == "accommodation":
            ...
            listings = Tour.objects.filter(id__in=listing_ids, host=request.user)
            count = listings.count()
            listings.delete()
        else:
            return JsonResponse(
...
def view_accommodation(request, listing_id):
    ...
    try:
        accommodation = get_object_or_404(
            Accommodation, id=listing_id, host=request.user
        )
        ...
        context = {
            ...
            "amenities_list": (
                accommodation.amenities.split(",") if accommodation.amenities else []
            ),
            "features_list": (
                accommodation.property_features.split(",")
                if accommodation.property_features
                else []
            ),
            "landmarks_list": (
                accommodation.nearby_landmarks.split(",")
                if accommodation.nearby_landmarks
                else []
            ),
        }
...
def edit_accommodation(request, listing_id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            accommodation.property_name = request.POST.get(
                "property_name", accommodation.property_name
            )
            accommodation.property_type = request.POST.get(
            ...
            accommodation.street_address = request.POST.get(
                "street_address", accommodation.street_address
            )
            accommodation.num_rooms = int(
                request.POST.get("num_rooms", accommodation.num_rooms)
            )
            accommodation.beds_per_room = int(
                request.POST.get("beds_per_room", accommodation.beds_per_room)
            )
            accommodation.bed_type = request.POST.get(
                "bed_type", accommodation.bed_type
            )
            accommodation.num_bathrooms = int(
                request.POST.get("num_bathrooms", accommodation.num_bathrooms)
            )
            accommodation.max_guests = int(
                request.POST.get("max_guests", accommodation.max_guests)
            )
            accommodation.tagline = request.POST.get("tagline", accommodation.tagline)
            accommodation.full_description = request.POST.get(
                "full_description", accommodation.full_description
            )
            accommodation.base_price = float(
                request.POST.get("base_price", accommodation.base_price)
            )
            ...
            accommodation.cancellation_policy = request.POST.get(
                "cancellation_policy", accommodation.cancellation_policy
            )
            accommodation.house_rules = request.POST.get(
...
def edit_tour(request, listing_id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            tour.min_participants = int(
                request.POST.get("min_participants", tour.min_participants)
            )
            tour.max_participants = int(
                request.POST.get("max_participants", tour.max_participants)
            )
            tour.age_restrictions = request.POST.get(
                "age_restrictions", tour.age_restrictions
            )
            tour.tagline = request.POST.get("tagline", tour.tagline)
            tour.full_description = request.POST.get(
            ...
            tour.price_per_person = float(
                request.POST.get("price_per_person", tour.price_per_person)
            )
            tour.fitness_level = request.POST.get("fitness_level", tour.fitness_level)
            tour.cancellation_policy = request.POST.get(
...
def update_accommodation(request, listing_id):
    ...
    try:
        ...
        messages.success(
            request, f'"{accommodation.property_name}" has been updated and refreshed!'
        )
        return redirect("view_accommodation", listing_id=listing_id)
...
def genius_rewards(request):
    ...
    Now uses real data from GeniusProfile model
    """
    from .views_genius import GeniusRewardsView

    # Use the class-based view to get all the context
    view = GeniusRewardsView()
    view.request = request
    context = view.get_context_data()

    # Keep backward compatibility with old template variables (if needed)
    genius_profile = context.get("genius_profile")
    if genius_profile:
        context.update(
            {
                "genius_level": genius_profile.level,
                "points": genius_profile.total_points,
                "next_level_points": genius_profile.next_level_points or "Max Level",
                "stays_completed": genius_profile.total_bookings,
                "total_savings": context.get("total_savings", 0),
                "benefits": [
                    {
                        "name": f"{genius_profile.discount_percentage}% discount on stays",
                        "active": True,
                    },
                    {
                        "name": f"{genius_profile.points_multiplier}x points multiplier",
                        "active": True,
                    },
                    {
                        "name": "Priority customer support",
                        "active": genius_profile.level >= 2,
                    },
                    {"name": "Free room upgrades", "active": genius_profile.level >= 2},
                    {"name": "VIP treatment", "active": genius_profile.level >= 3},
                ],
                "available_rewards": [
                    {"title": "$50 Travel Credit", "points": 500, "available": True},
                    {
                        "title": "Premium Upgrade Voucher",
                        "points": 750,
                        "available": True,
                    },
