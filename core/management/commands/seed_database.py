...
from core.models import (
    UserProfile, Accommodation, AccommodationPhoto,
    Tour, TourPhoto, Country
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'   ✅ Loaded {len(demo_attractions)} country attractions'))
    ...
    def seed_countries(self, countries_data, demo_attractions):
        ...
        for country_slug, country_info in countries_data.items():
            ...
            country, created = Country.objects.update_or_create(
                slug=country_slug,
                defaults={
                    'name': country_info.get('name', country_slug.replace('-', ' ').title()),
            ...
            for attraction_data in attractions_list:
                ...
                full_attraction = {
                    ...
                    'description': attraction_details.get('description', attraction_desc),
                    'short_description': attraction_details.get('short_description', attraction_desc),
                    ...
                    'photos': attraction_details.get('photos', [attraction_img] if attraction_img else []),
                    'historical_significance': attraction_details.get('historical_significance', ''),
                    ...
                    'best_time_to_visit': attraction_details.get('best_time_to_visit', ''),
                    ...
                    'nearby_attractions': attraction_details.get('nearby_attractions', []),
        ...
        self.stdout.write(self.style.SUCCESS(f'   • Countries updated: {countries_updated}'))
        self.stdout.write(self.style.SUCCESS(f'   • Total countries: {countries_created + countries_updated}'))
        self.stdout.write(self.style.SUCCESS(f'   • Total attractions seeded: {total_attractions}'))

    def seed_accommodations(self, demo_user):
        """Seed sample accommodations - to be implemented"""
        self.stdout.write(self.style.WARNING('\n🏨 Seeding accommodations...'))
        self.stdout.write(self.style.WARNING('   ⚠️  Accommodation seeding not yet implemented'))
